...
async function generateSingleReportExcel(report) {
 ...
 const endRowIndex = startRowIndex + days.length - 1;
  ...
  worksheet.columns = [
    ...
    { width: 28 }, // L: פירוט הפעילות
    { width: 14 }, // M: שעות מקוריות
    { width: 22 }  // N: הערת מנחה
  ];
...
/**
 * Generates an RTL Summary Excel file for all district reports
 */
 ...
 ExcelJS = require('exceljs');
  ...
  reports.forEach((rep, idx) => {
    ...
    for (let c = 1; c <= 11; c++) {
      const cell = row.getCell(c);
      cell.font = { name: 'Arial', size: 9 };
      cell.alignment = { vertical: 'middle', horizontal: [1, 3, 4, 7, 8, 9, 10, 11].includes(c) ? 'center' : 'right' };
      cell.border = {
        top: { style: 'thin', color: { argb: 'E0E0E0' } },
        bottom: { style: 'thin', color: { argb: 'E0E0E0' } },
        left: { style: 'thin', color: { argb: 'E0E0E0' } },
